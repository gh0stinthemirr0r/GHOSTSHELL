...
//! 
//! Provides real-time system telemetry including CPU, memory, disk, and uptime

use crate::{DashError, Result};
use chrono::{DateTime, Utc};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use sysinfo::{System, Cpu, Disk, Networks, Processes};
...
impl SystemMonitor {
    ...
    pub fn get_system_info(&mut self) -> Result<SystemInfo> {
        ...
        let disks: Vec<DiskInfo> = self.system.disks()
            .iter()
            .map(|disk| {
        ...
        let mut network_summary = NetworkSummary {
        ...
        for (_, network) in networks {
            network_summary.total_bytes_received += network.received();
            network_summary.total_bytes_transmitted += network.transmitted();
            network_summary.total_packets_received += network.packets_received();
            network_summary.total_packets_transmitted += network.packets_transmitted();
            network_summary.total_errors_received += network.errors_on_received();
            network_summary.total_errors_transmitted += network.errors_on_transmitted();
        }
    ...
    /// Get historical system metrics (placeholder for future implementation)
    pub fn get_historical_metrics(&self, _duration_hours: u32) -> Result<Vec<SystemMetric>> {
...
/// Historical system metric point
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    async fn test_system_info_collection() {
        ...
        let system_info = monitor.get_system_info().unwrap();
    ...
    #[tokio::test]
    async fn test_system_alerts() {
        let mut monitor = SystemMonitor::new();
        let alerts = monitor.get_system_alerts().unwrap();
